...
echo "corrected pointing takes effect (from $WORKSPACE,"
echo "with MARS_CONFIG_PATH=$CALIB_DIR exported):"
echo "  tig marsmos inp=frames.lis out=mosaic.img navtable=pointing.nav"
echo "  tig env HWLOC_COMPONENTS=-x86 marsmap inp=frames.lis out=mosaic.img navtable=pointing.nav"
echo ""
echo "marsmap calls MPI_Init, whose hwloc CPU detection divides by zero on some"
echo "hosts (exit 136, no output); HWLOC_COMPONENTS=-x86 avoids it. marsmos does"
echo "not need it."
echo ""
